...
public final class Conscrypt {
    ...
    public static void echPbuf(String msg, byte[] buf) {
        ...
        for (int i = 0; i < blen; i++) {
            if ((i != 0) && (i % 16 == 0))
                System.out.print("\n    ");
            System.out.print(String.format("%02x:", buf[i] & 255));
        }
        System.out.print("\n");
    }
    ...
    public static byte[] getEchConfigListFromDns(String hostname) {
        return getEchConfigListFromDns(hostname, 443);
    }

    public static byte[] getEchConfigListFromDns(String hostname, int port) {
        String dnshost = hostname;
        if (port != 443) {
            dnshost = "_" + port + "._https." + hostname; // query for non-standard port
        }
        return Platform.getEchConfigListFromDns(dnshost);
    }
